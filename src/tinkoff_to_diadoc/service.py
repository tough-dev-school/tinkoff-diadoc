...
class TinkoffToDiadoc:
    ...
    def get_my_company(self) -> DiadocLegalEntity:
    ...
    def act(self) -> None:
        my_company = self.get_my_company()
        bank_accounts = self.get_bank_accounts()
        recent_payers = self.get_recent_distinct_payers(bank_accounts, my_company)
        counteragents = self.get_counteragents(my_company)
